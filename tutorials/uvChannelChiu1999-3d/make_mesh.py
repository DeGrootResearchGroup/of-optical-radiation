#!/usr/bin/env python3
"""Generate the 3-D channel blockMeshDict for the Chiu et al. 1999 pilot UV reactor.
...
at Southport AWT Plant, Table 1 p.11 and Fig. 2 p.11):
  ...
  * Open channel, 33 cm wide, water depth 76 cm. 3-D extrusion of the
    2-D parent case (tutorials/uvChannelChiu1999/). The channel floor
    at z = 0 is a no-slip wall, the free surface at z = 0.76 m is a
    slip wall (rigid-lid approximation; surface waves not resolved).
...
WATER_DEPTH     = 0.76          # m  (Chiu pilot; lamps span the full
                                #     depth from channel floor to free
                                #     surface)

# 1 m upstream fetch is enough in 3-D (faster BL development from
# 3-D dissipation pathways than the equivalent 2-D simulation, which
# uses 2 m). OUTLET_BUFFER = 0.25 m: just clearance for particle
# escape, no flow development needed.
INLET_BUFFER    = float(os.environ.get('CHIU_INLET_BUFFER',  '1.0'))
OUTLET_BUFFER   = float(os.environ.get('CHIU_OUTLET_BUFFER', '0.25'))

# Vertical cell size. 1 cm gives 76 cells across the water depth and
# puts y+ ~ 50 at the floor first cell -- the log-law region that
# nutUSpaldingWallFunction is most accurate in.
TARGET_CELL_SIZE_Z = float(os.environ.get('CHIU_TARGET_CELL_SIZE_Z', '0.01'))
...
# 3-D z-extrusion. One vertex layer at z=0 (channel floor) and one at
# z=WATER_DEPTH (free surface), with cells of size TARGET_CELL_SIZE_Z
# subdividing the block.
z_vertices = np.array([0.0, WATER_DEPTH])
...
ndz = max(1, int(round(WATER_DEPTH / TARGET_CELL_SIZE_Z)))

print(f"Block grid: {len(x_vertices) - 1} x {len(y_vertices) - 1} blocks")
print(f"z-cells per block: {ndz}  (water depth {WATER_DEPTH:.2f} m / "
      f"target cell size {TARGET_CELL_SIZE_Z:.3f} m)")
print(f"Approx cell count: {int(ndx.sum() * ndy.sum() * ndz)}")
...
# Channel floor at z=0: a regular no-slip wall.
floor        = BoundaryTag('floor',        type_='wall')
# Free surface at z=WATER_DEPTH: a slip wall (zero shear). Type is
# 'wall' so it goes through the dose tracker's hitWallPatch
# (specular reflection of particles), which is the right behaviour
# for the plan-view physics where particles slide along the surface.
# The slip flow physics is set in 0/U etc. by BC type, not patch type.
free_surface = BoundaryTag('freeSurface', type_='wall')

struct.boundary_tags[0,  :, :, 0] = inlet         # x-min
struct.boundary_tags[-1, :, :, 0] = outlet        # x-max
struct.boundary_tags[:, 0,  :, 1] = walls         # y-min (side wall)
struct.boundary_tags[:, -1, :, 1] = walls         # y-max (side wall)
struct.boundary_tags[:, :, 0,  2] = floor         # z-min (channel floor)
struct.boundary_tags[:, :, -1, 2] = free_surface  # z-max (free surface)
